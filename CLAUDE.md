...
- Two ways in: `/contacto` (3-step brief, demo grátis) and `/pedido` (detailed request for people who skip the demo; questions per service live in `order` in content.ts). `/pedido` posts to the Vercel function `api/pedido.js`, which emails via Resend (env RESEND_API_KEY, optional PEDIDO_TO/PEDIDO_FROM); without the key or on error it falls back to a prefilled mailto. `/contacto` still uses mailto only. Weld phone is `site.phone` (932 803 139), shown on /pedido, /contacto and the footer.
